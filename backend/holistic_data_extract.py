...
with mp_holistic.Holistic(
    ...
    while cap.isOpened():
        ...
        #ประมวลผลภาพ
        #แปลงสีภาพ BGR -> RGB + ล็อกภาพ
        image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        image.flags.writeable = False
        #ส่งภาพให้ mediapipe ประมวลผล keypoints
        results = holistic.process(image)
        #ปลดล็อกและแปลงสีกลับ
        ...
        if key == ord('r'): # กด r อัด
            is_recording = True
            print("เริ่มบันทึกข้อมูล...")
        elif key == ord('s'): # กด s หยุดอัด
            is_recording = False
            print("หยุดบันทึกข้อมูล...")
        elif key == ord('q'): # กด q ออก
            break
